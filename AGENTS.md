...
> Generated: 2026-08-07 · Commit: `07d1e99b` · Branch: `main`
...
| `migration/recreate-dev-pantella-wow` | Feature branch — fork-specific work (was `dev/pantella-wow`, renamed for clarity) | No |
| `archive/dev-pantella-wow-pre-main-rebase-2026-07-16` | Preserved orphan history from the July 2026 rebase reset | No |

**Workflow:**
1. `main` is synced from upstream via the scheduled `sync-upstream.yml` workflow (PR-based: creates a sync PR, user merges with admin bypass if conflict).
2. Fork-specific work (WoW integration, security audits, docs) goes on `migration/recreate-dev-pantella-wow`.
3. `migration/recreate-dev-pantella-wow` is kept current with `main` via merge PR (NOT rebase — the `Require Approvals` ruleset forbids force pushes).
4. Short-lived `feat/*` branches for active development, PR'd back into `migration/recreate-dev-pantella-wow`.

**Why merge (not rebase):** The `Require Approvals (dev/pantella-wow)` ruleset is PR-only. Rebase requires force-push. Merge commits work with PR-only protection and preserve history. The July 2026 Codex session proved this experimentally — a rebase attempt failed with "no common ancestor" because the branch had been squashed. The one-time reset (disabling + force-with-lease) is preserved as `archive/dev-pantella-wow-pre-main-rebase-2026-07-16`.

See `~/Projects/Pantella/.omo/plans/fork-sync-workflow.md` for the full operational guide.
...
The WoW addon has its own repository (`niStee/pantella-wow`) and is **included as a Git submodule** at `addons/pantella-wow/` (see `.gitmodules`). After cloning this repo, run `git submodule update --init --recursive` to populate the submodule directory. The `.gitignore` covers `/addons/pantella-wow/` so the standalone clone doesn't show up as untracked. To work on the addon independently, clone it to a separate path (e.g. `~/Projects/pantella-wow` on Linux/macOS, `D:\repos\pantella-wow` on Windows).
